...
pub const ExpertWeights = struct {
    ...
    /// Load weights from file (memory-mapped for zero-copy)
    pub fn loadFromFile(self: *ExpertWeights, path: []const u8) !void {
        ...
        const float_data: [*]const f32 = @ptrCast(@alignCast(mapped.ptr));
    ...
    /// Matrix-vector multiplication with optional SIMD
...
test "expert weights forward pass" {
    ...
    // Output should be non-zero (random weights)
